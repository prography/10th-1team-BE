...
class GeoRectSliceService(
    ...
    init {
        val stream = this::class.java.classLoader.getResourceAsStream(RESOURCE_PATH)
     ...
     */
    fun sliceRectFromFeature(admName: String, step: Double): List<String> {
        ...
        while (x < maxX) {
            ...
            while (y < maxY) {
                val x2 = x + step
                val y2 = y + step
                val cell = geometryFactory.createPolygon(
                    arrayOf(
                        Coordinate(x, y),
                        Coordinate(x2, y),
                        Coordinate(x2, y2),
